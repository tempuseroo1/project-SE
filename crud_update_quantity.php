...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        // This part is similar to the crud_create_product.php, but instead we update a record and not insert
...
<div class="content update">
    ...
    <form action="crud_update_quantity.php?id=<?=$product['id']?>" method="post">
        ...
        <input type="number" step="0.01" name="price" placeholder="Price" value="<?=$product['price']?>" id="price" hidden>
        <input type="number" step="0.01" name="rrp" placeholder="rrp" value="<?=$product['rrp']?>" id="rrp" hidden>
        <label for="quantity">Quantity</label>
        <input type="number" name="quantity" placeholder="Quantity" value="<?=$product['quantity']?>" id="quantity">
        <label for="img">Image</label>
        <input type="text" name="img" placeholder="Image" value="<?=$product['img']?>" id="img">
